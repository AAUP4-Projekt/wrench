...
use super::{environment::{env_get, EnvironmentCell, ExpressionValue}, evaluate::evaluate_expression, library::import_csv, table::{Row, Table, TableCellType}};
...
fn pipe_rollout(expr: Box<Expr>, function_name: String, args: Vec<Box<Expr>>, env: &mut Vec<Vec<EnvironmentCell>>) -> Vec<SimplePipe> {
    let mut result: Vec<SimplePipe> = Vec::new();

    for(arg) in args.iter() {
        let evaluated_args = args.iter().map(|arg| evaluate_expression(*arg.clone(), env)).collect::<Vec<ExpressionValue>>();
